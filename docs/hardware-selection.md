...
- 显示屏：Waveshare 树莓派生态 800x480 DSI IPS 电容触摸屏
...
800x480 屏幕对 GBA 非常友好。GBA 原生分辨率为 240x160，按 3 倍整数缩放后为 720x480，刚好占满 480 垂直像素，左右各留 40 像素黑边，可获得清晰、无纵向拉伸的显示效果。

但当前验证组合存在一个需要优先确认的风险：Radxa CM3 模组支持 MIPI DSI，但 Radxa CM3 IO Board 官方规格主要列出 HDMI 和 eDP 显示输出；Waveshare 树莓派 DSI 屏并不一定能直接接到 CM3 IO Board 上即用。建议把显示接口兼容性作为第一阶段 bring-up 的最高优先级。
...
- CM3 IO Board 未必直接暴露树莓派 DSI 屏所需的 MIPI DSI 接口。
...
### 5.1 推荐方向：800x480 DSI IPS 电容屏

800x480 是本项目适配 GBA 的优选分辨率：
...
当前组合 “Radxa CM3 + Radxa CM3 IO Board + Waveshare DSI 屏” 不应默认认为可以直连成功。建议按以下顺序确认：

1. 查 CM3 IO Board 原理图，确认是否实际引出 MIPI DSI。
2. 查 Waveshare 屏幕 FPC/转接板 pinout，确认 DSI、电源、I2C、背光引脚。
3. 比对 CM3 IO Board 接口形态与 Waveshare 屏幕线缆形态。
4. 若 IO Board 仅暴露 HDMI/eDP，则优先用 HDMI/eDP 屏完成系统和模拟器验证。
5. 若必须验证 DSI 屏，则需要更换能引出 DSI 的载板，或设计临时转接/专用底板。
...
| 开发底板 | Radxa CM3 IO Board | 系统 bring-up、外设验证 | 可能不直接支持目标 DSI 屏 |
| 屏幕 | Waveshare 800x480 DSI IPS 电容屏 | 目标显示验证 | DSI pinout、驱动、设备树适配 |
...
1. DSI 屏与 CM3 IO Board 的物理和电气兼容性不明确。
...
- 主线 B：单独攻关 800x480 DSI 屏的硬件连接和设备树适配，确认后再合并到掌机方案。

如果确认 CM3 IO Board 无法直接接 Waveshare DSI 屏，不建议在开发板阶段投入过多机械转接成本。更合理的路径是先用可用显示输出完成软件验证，同时为下一版专用底板预留正确的 DSI、触摸和背光接口。
...
- 确认是否能通过 CM3 IO Board 引出 MIPI DSI。
- 若 DSI 路径可行，开始设备树适配；若不可行，规划专用底板或替换屏幕方案。
